var AWS =
{
	...
	ajax_auth_login: function(callback) {
		// 第三方登录，关闭当前窗口
		if(window.opener) {
	...
	ajax_processer: function (type, result)
	{
		...
		else
		{
			...
			if (type=='login_' &&result.rsm && result.rsm.url)
			{
				//console.log(1111,type,result.rsm.url);
				//exit;
				// 第三方登录，关闭当前窗口
				AWS.ajax_auth_login(function() {
			...
			else
			{
				...
				switch (type)
				{
					...
					case 'error_message':
						...
						if(result.rsm!=null&&result.rsm['url'])
						{
							//console.log(1,decodeURIComponent(result.rsm.url));
							window.location = decodeURIComponent(result.rsm.url);
						}else{
					...
					case 'reply_question':
						...
						if (result.rsm!=null&&result.rsm.ajax_html)
						{
						...
						else if(result.rsm!=null&&result.rsm.url)
						{
							window.location = decodeURIComponent(result.rsm.url);
						}
						else
